...
class DemoButtonBar extends StatelessWidget {
  ...
  DemoButtonBar(this.data, {Key key, this.selectedItem, this.selectedCallback})
      : super(key: key);
...
class DemoButtonBarDisplay extends LeafRenderObjectWidget {
  ...
  void updateRenderObject(
      BuildContext context, covariant DemoButtonBarRenderObject renderObject) {
    renderObject
      ..data = data
...
class DemoButtonBarRenderObject extends RenderBox {
  ...
  void beginFrame(Duration timeStamp) {
    ...
    _selectedIndex +=
        (_targetSelectedIndex - _selectedIndex) * min(1.0, elapsed * 15.0);
  ...
  void performResize() {
    size = new Size(
        constraints.constrainWidth(), constraints.constrainHeight(40.0));
  }
  ...
  void performLayout() {
    ...
    for (int i = 0; i < _data.length; i++) {
      ...
      DataLabelParagraph dlp = new DataLabelParagraph()
        ..paragraph = paragraph
        ..item = label
        ..size = new Size(boxes.last.right - boxes.first.left,
            boxes.last.bottom - boxes.first.top);
      _labelParagraphs[i] = dlp;
    }

    _fullParagraphWidth = _labelParagraphs.fold<double>(0.0,
        (double width, DataLabelParagraph p) {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    canvas.drawRRect(
        new RRect.fromRectAndRadius(offset & size, const Radius.circular(20.0)),
        new ui.Paint()..color = const Color.fromARGB(77, 0, 0, 0));
    if (_data == null) {
      return;
    }
    ...
    for (DataLabelParagraph p in _labelParagraphs) {
      xs.add(new Offset(x + edgePadding,
          p.size.width + DemoButtonPadding * 2 - edgePadding * 2));
    ...
    canvas.drawRRect(
        new RRect.fromRectAndRadius(
            new Offset(dx, offset.dy + edgePadding) &
                new Size(width, size.height - edgePadding * 2),
            const Radius.circular(20.0)),
        new ui.Paint()..color = const Color.fromARGB(255, 87, 165, 244));

    x = offset.dx;
    for (DataLabelParagraph p in _labelParagraphs) {
      canvas.drawParagraph(
          p.paragraph,
          new Offset(x + DemoButtonPadding,
              offset.dy + size.height / 2.0 - p.size.height / 2.0));
      x += p.size.width + spacing + DemoButtonPadding * 2;
    }
  }
