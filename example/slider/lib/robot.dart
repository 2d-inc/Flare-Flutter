...
class NimaWidget extends LeafRenderObjectWidget
{
	...
	NimaWidget(this._filename, {Key key, this.scrollOffset, this.touchPosition}): super(key: key);
...
class NimaRenderObject extends RenderBox
{
	...
	void beginFrame(Duration timeStamp) 
	{
		...
		if(_actorInstance != null && _scrollOffset == 0.0)
		{
			...
			if(_animation != null)
			{
				_animation.apply(_animationTime%_animation.duration, _actorInstance, 1.0);
			}
	...
	void paint(PaintingContext context, Offset offset)
	{
		...
		if(_actorInstance == null || (_scrollOffset >= 1.0 || _scrollOffset <= -1.0))
		{
			return;
		}
		canvas.save();
		...
		double scale = _isControlled ? zoomedIn : max(size.width/width, size.height/(height-heightPadding));
		...
		canvas.drawRect(new Offset(offset.dx, offset.dy + (size.height-fadeHeight)) & new Size(size.width, fadeHeight), 
								new ui.Paint()	..shader = new ui.Gradient.linear(new Offset(0.0, offset.dy + (size.height-fadeHeight)), new Offset(0.0, offset.dy + fadeHeight), <Color>[const Color.fromARGB(0, 0, 0, 0), const Color.fromARGB(128, 0, 0, 0)])
	...
	set touchPosition(Offset t)
	{
		...
		if(t != _touchPosition)
		{
			_touchPosition = t ?? new Offset(ui.window.physicalSize.width/2, ui.window.physicalSize.height/2);
		}
	}
}