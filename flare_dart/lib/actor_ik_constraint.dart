...
class ActorIKConstraint extends ActorTargetedConstraint {
  ...
  void completeResolve() {
    ...
    _fkChain = List<BoneChain?>.filled(count, null, growable: false);
    int idx = count - 1;
    while (end != start!.parent) {
  ...
  static ActorIKConstraint read(ActorArtboard artboard, StreamReader reader,
    ...
    if (numInfluencedBones > 0) {
      component._influencedBones = List<InfluencedBone?>.filled(
          numInfluencedBones, null,
          growable: false);
  ...
  void copyIKConstraint(ActorIKConstraint node, ActorArtboard artboard) {
    ...
    if (node._influencedBones != null) {
      _influencedBones = List<InfluencedBone?>.filled(
          node._influencedBones!.length, null,
          growable: false);
