      ...
      for (FlutterFill fill in _fills) {
        ...
        if (overrideColor != null) {
          paint.color = overrideColor.withOpacity(
              overrideColor.opacity * paint.color.opacity);
        }
...
class FlutterColorFill extends ColorFill
    implements FlutterFill {
  ui.Paint getPaint(Float64List transform, double modulateOpacity) {
    ui.Paint paint = new ui.Paint()
      ..color = new ui.Color.fromRGBO(
          (color[0] * 255.0).round(), (color[1] * 255.0).round(),
          (color[2] * 255.0).round(), color[3] * modulateOpacity * opacity)
...
class FlutterColorStroke extends ColorStroke
...
class FlutterGradientFill extends GradientFill
    implements FlutterFill {
  ui.Paint getPaint(Float64List transform, double modulateOpacity) {
    ...
    for (int i = 0; i < numStops; i++) {
      ui.Color color = new ui.Color.fromRGBO((colorStops[idx] * 255.0).round(),
          (colorStops[idx + 1] * 255.0).round(),
          (colorStops[idx + 2] * 255.0).round(), colorStops[idx + 3]);
      colors.add(color);
    ...
    ui.Paint paint = new ui.Paint()
      ...
      ..shader = new ui.Gradient.linear(
          new ui.Offset(renderStart[0], renderStart[1]),
          new ui.Offset(renderEnd[0], renderEnd[1]), colors, stops)
      ..style = ui.PaintingStyle.fill;
    return paint;
  }
...
class FlutterGradientStroke extends GradientStroke
    implements FlutterStroke {
  ui.Paint getPaint(Float64List transform, double modulateOpacity) {
    List<ui.Color> colors = new List<ui.Color>();
    List<double> stops = new List<double>();
...
class FlutterRadialFill extends RadialGradientFill
    implements FlutterFill {
  ui.Paint getPaint(Float64List transform, double modulateOpacity) {
    ...
    ui.Gradient radial = new ui.Gradient.radial(
        Offset(renderStart[0], renderStart[1]), radius, colors, stops,
...
class FlutterActorEllipse extends ActorEllipse
    with FlutterPathPointsPath {
  ActorComponent makeInstance(ActorArtboard resetArtboard) {
...
class FlutterActorPolygon extends ActorPolygon
    with FlutterPathPointsPath {
  ActorComponent makeInstance(ActorArtboard resetArtboard) {
...
class FlutterActorStar extends ActorStar
    with FlutterPathPointsPath {
  ActorComponent makeInstance(ActorArtboard resetArtboard) {
...
class FlutterActorRectangle extends ActorRectangle
    with FlutterPath {
  ActorComponent makeInstance(ActorArtboard resetArtboard) {
...
class FlutterActorTriangle extends ActorTriangle
    with FlutterPathPointsPath {
  ActorComponent makeInstance(ActorArtboard resetArtboard) {
...
abstract class FlutterPathPointsPath
    implements FlutterPath {
  ui.Path _path;
        ...
        case PointType.Straight:
          {
            ...
            if (radius > 0) {
              if (!isClosed && (i == 0 || i == pl - 1)) {
                ...
                Vec2D nextPoint = next is CubicPathPoint ? next.inPoint : next
                    .translation;
                ...
                double renderRadius = min(
                    toPrevLength, min(toNextLength, radius));

                Vec2D translation = Vec2D.scaleAndAdd(
                    new Vec2D(), pos, toPrev, renderRadius);
                renderPoints.add(new CubicPathPoint.fromValues(
                    translation, translation, Vec2D.scaleAndAdd(
                    new Vec2D(), pos, toPrev, iarcConstant * renderRadius)));
                translation =
                    Vec2D.scaleAndAdd(new Vec2D(), pos, toNext, renderRadius);
                previous = new CubicPathPoint.fromValues(translation,
                    Vec2D.scaleAndAdd(
                        new Vec2D(), pos, toNext, iarcConstant * renderRadius),
                    translation);
