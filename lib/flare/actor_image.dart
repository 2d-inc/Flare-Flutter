...
class ActorImage extends ActorNode
  ...
  static ActorImage readSequence(ActorArtboard artboard, StreamReader reader,
      ActorImage node) {
    ...
    if (node._textureIndex != -1) {
      ...
      Float32List uvs = new Float32List(
          node._vertexCount * 2 * frameAssetCount);
      ...
      for (int i = 1; i < frameAssetCount; i++) {
        ...
        SequenceFrame frame = new SequenceFrame(
            reader.readUint8("atlas"), offset * 4);
  ...
  void copyImage(ActorImage node, ActorArtboard resetArtboard) {
    ...
    if (node._animationDeformedVertices != null) {
      ...
      new Float32List.fromList(node._animationDeformedVertices);
    }
  ...
  void updateVertexPositionBuffer(Float32List buffer,
    ...
    if (isSkinned) {
      ...
      Float32List influenceMatrix = new Float32List.fromList(
      ...
      for (int i = 0; i < _vertexCount; i++) {
        ...
        x = influenceMatrix[0] * px + influenceMatrix[2] * py +
            influenceMatrix[4];
        y = influenceMatrix[1] * px + influenceMatrix[3] * py +
            influenceMatrix[5];
  ...
  AABB computeAABB() {
    ...
    Mat2D worldTransform = this.worldTransform;
    return new AABB.fromValues(
        worldTransform[4], worldTransform[5], worldTransform[4],
        worldTransform[5]);
  }
}