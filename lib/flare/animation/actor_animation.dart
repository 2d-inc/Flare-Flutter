...
class ComponentAnimation {
  ...
  static ComponentAnimation read(StreamReader reader,
...
class ActorAnimation {
          ...
          case PropertyTypes.Trigger:
            ...
            if (idx == 0) {
              if (kfl > 0 && keyFrames[0].time == toTime) {
                ActorComponent component = components[keyedComponent
                    .componentIndex];
                triggerEvents.add(new AnimationEventArgs(
                    component.name, component, property.propertyType, toTime,
                    0.0));
              }
            }
            else {
              for (int k = idx - 1; k >= 0; k--) {
                ...
                if (frame.time > fromTime) {
                  ActorComponent component = components[keyedComponent
                      .componentIndex];
                  triggerEvents.add(new AnimationEventArgs(
                      component.name, component, property.propertyType,
                      frame.time, toTime - frame.time));
                  /*triggered.push({
										name:component._Name,
										component:component,
										propertyType:property._Type,
  ...
  static ActorAnimation read(StreamReader reader,
      List<ActorComponent> components) {
    ...
    List<ComponentAnimation> animatedComponents = new List<ComponentAnimation>(
        numKeyedComponents);
    for (int i = 0; i < numKeyedComponents; i++) {
      ...
      if (componentAnimation != null) {
        ActorComponent actorComponent = components[componentAnimation
            .componentIndex];
    ...
    new List<ComponentAnimation>(animatedComponentCount);
    animation._triggerComponents =
    new List<ComponentAnimation>(triggerComponentCount);
