...
class JellyComponent extends ActorComponent {
  ...
  List<Vec2D> normalizeCurve(List<Vec2D> curve, int numSegments) {
    ...
    for (int i = 1; i <= numSegments; i++) {
      ...
      points.add(new Vec2D.fromValues(
  ...
  void resolveComponentIndices(List<ActorComponent> components) {
    ...
    if (bone != null) {
      ...
      if (firstBone != null) {
        ...
        if (_outTarget == null && firstBone.jelly != null &&
    ...
    Set<ActorConstraint> constraints = new Set<ActorConstraint>.from(
  ...
  void updateJellies() {
    ...
    if (fuzzyEquals(_cachedTip, tipPosition) &&
        ...
        fuzzyEquals(_cachedIn, _inPoint) && _cachedScaleIn == _scaleIn &&
  ...
  void update(int dirt) {
    ...
    if (_inTarget != null) {
      ...
      if (firstBone == bone && parentBoneJelly != null &&
        ...
        Vec2D localParentOut = Vec2D.transformMat2D(
            new Vec2D(), translation, inverseWorld);
    ...
    if (_outTarget != null) {
      ...
      if (firstBoneJelly != null && firstBoneJelly._inTarget != null) {
        ...
        Vec2D worldChildInDir = Vec2D.subtract(
            new Vec2D(), firstBone.getWorldTranslation(new Vec2D()),
            translation);
      ...
      Vec2D scaledOut = Vec2D.scale(
          new Vec2D(), _outDirection, _easeOut * bone.length * CurveConstant);
