...
class FlareActor extends LeafRenderObjectWidget {
  ...
  FlareActor(this.filename, {this.boundsNode, this.animation, this.fit = BoxFit
      .contain, this.alignment = Alignment
      .center, this.isPaused = false, this.controller, this.callback, this.color, this.shouldClip = true});
...
class FlareActorRenderObject extends RenderBox {
  ...
  void beginFrame(Duration timestamp) {
    ...
    final double t = timestamp.inMicroseconds /
        Duration.microsecondsPerMillisecond / 1000.0;
    ...
    for (int i = 0; i < _animationLayers.length; i++) {
      ...
      lastMix = (_mixSeconds == null || _mixSeconds == 0.0) ? 1.0 : min(
          1.0, layer.mix / _mixSeconds);
    ...
    if (animationName == null && _animationLayers.length == 1 &&
        lastMix == 1.0) {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (_artboard != null) {
      ...
      double x = -bounds[0] - contentWidth / 2.0 -
          (_alignment.x * contentWidth / 2.0);
      double y = -bounds[1] - contentHeight / 2.0 -
      ...
      switch (_fit) {
        ...
        case BoxFit.contain:
          double minScale = min(
              size.width / contentWidth, size.height / contentHeight);
